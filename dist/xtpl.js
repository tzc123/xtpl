...
var Section = __webpack_require__(329);
var render = __webpack_require__(331);
...
var match = __webpack_require__(330);
...
var Section = function () {
  ...
  _createClass(Section, [{
    key: "deal",
    value: function deal(obj) {
      var logic = match(obj.value);
      var value = _extends({}, obj, logic);
    ...
    key: "createVariable",
    value: function createVariable(obj) {
      var current = this.current;
    ...
    value: function start(_start) {
      this.current.start = _start;
      console.log(_start.type, "开始");
    }
  }, {
    key: "end",
    value: function end(_end) {
      this.current.end = _end;
      console.log(_end.type, "闭合");
    }
  }, {
    key: "middle",
    value: function middle(_middle) {
      this.current.middle = _middle;
    }
  }, {
    key: "init",
    value: function init() {
    ...
    value: function search() {
      ...
      while (true) {
        var tagBegin = this.tpl.indexOf("{", startIndex);
        var tagClose = this.tpl.indexOf("}", startIndex);
        if (tagBegin == -1) {
          if (tagClose == -1) {
          ...
          if (tagClose == -1) {
            ...
            var value = this.tpl.substring(tagBegin + 1, tagClose);
...
// 关键字
var kw = ['for', 'elseif', 'else', 'if'];

function match(str) {
  var logic = void 0;
  kw.some(function (item) {
  ...
  make(current, function (node) {
    if (node != node.belong.tree.root) {
      ...
      node.render = function (data) {
        ...
        this.variables && this.variables.forEach(function (variable) {
          content = content.replace("{" + variable.value + "}", eval(getVariable(data, variable.value).value));
        });
        return content;
      };
    }
...
function resolve(node) {
  ...
  node.oldContent = node.belong.tpl.substring(node.start.tagBegin, node.end.tagClose + 1);
  node.render = function (data) {
    var _this = this;

    console.log(data);
    var tpl = this.belong.tpl;
    var content = "";
    var variables = this.variables;
    if (this.start.type == "for") {
      var list = eval(getVariable(data, this.param.list).value);
      if (!list) {
        throw new Error("不存在" + this.param.list);
      }
      if ({}.toString.call(list) == "[object Array]") {
        content = list.map(function (item, index) {
          var ocontent = _this.content;
          _this.children && _this.children.forEach(function (child) {
            ocontent = ocontent.replace(child.oldContent, child.render(_extends({}, data, _defineProperty({}, _this.param.item, list[index]))));
          });
          variables && variables.forEach(function (variable) {
            if (variable.value.indexOf(_this.param.item) != -1) {
              var res = getVariable(data, variable.value.replace(_this.param.item, _this.param.list + "[" + index + "]"));
              var value = eval(res.value);
              ocontent = ocontent.replace("{" + variable.value + "}", value);
            } else {
              var _res = getVariable(data, variable.value);
              var _value = eval(_res.value);
              ocontent = ocontent.replace("{" + variable.value + "}", _value);
            }
          });
          return ocontent;
        });
      ...
      var children = this.children && this.children.map(function (child) {
        return {
      ...
      if (this.middle) {
        var choices = [this.start].concat(_toConsumableArray(this.middle));
        choices = choices.map(function (choice, index) {
          var ocontent = tpl.substring(choice.tagClose + 1, choices[index + 1] ? choices[index + 1].tagBegin : _this.end.tagBegin);
          return {
            content: ocontent,
            tag: choice
          };
        ...
        choices.some(function (choice) {
          ...
          if (choice.tag.type != "else") {
            if (eval(getVariable(data, choice.tag.value))) {
              ...
              variables && variables.forEach(function (variable) {
                var res = getVariable(data, variable.value);
                var value = eval(res.value);
                content = choice.content.replace("{" + variable.value + "}", value);
              });
              return true;
            }
          } else {
...
function getVariable(data, str) {
  var complete = true;
  var newStr = str.replace(/(?!"')([^\.\[\]\{\}\(\)\s]*)(?!"')/g, function (match, key) {
    if (data[key.trim()]) {
